<div class="page-title-bar">
     <h4 class="page-title-bar-heading">Administration</h4>
</div>
<div class="container-fluid">
<div class="row">
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 text-center">
        <div class="card">
            <div class="card-heading"><h4>President</h4></div>
            <div class="card-img"><img class="img" src="asset/img/male.png"></div>
            <div class="card-name"><h5><b>Kumar</b></h5></div>
            <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
        </div>
    </div>
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 text-center">
            <div class="card">
                    <div class="card-heading"><h4>General Secretary</h4></div>
                    <div class="card-img"><img class="img" src="asset/img/male.png"></div>
                    <div class="card-name"><h5><b>Kumar</b></h5></div>
                    <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
            </div>
    </div>
</div>

<div class="row">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
            <div class="card-heading long-heading"><h4>Vice President</h4></div>
            <div class="card">
                    <div class="card-img"><img class="img" src="asset/img/male.png"></div>
                    <div class="card-name"><h5><b>Kumar</b></h5></div>
                    <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
            </div>

            <div class="card">
                    <div class="card-img"><img class="img" src="asset/img/male.png"></div>
                    <div class="card-name"><h5><b>Kumar</b></h5></div>
                    <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
            </div>

            <div class="card">
                    <div class="card-img"><img class="img" src="asset/img/male.png"></div>
                    <div class="card-name"><h5><b>Kumar</b></h5></div>
                    <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
            </div>

            <div class="card">
                    <div class="card-img"><img class="img" src="asset/img/male.png"></div>
                    <div class="card-name"><h5><b>Kumar</b></h5></div>
                    <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
            </div>

            <div class="card">
                    <div class="card-img"><img class="img" src="asset/img/male.png"></div>
                    <div class="card-name"><h5><b>Kumar</b></h5></div>
                    <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
            </div>

            <div class="card">
                    <div class="card-img"><img class="img" src="asset/img/male.png"></div>
                    <div class="card-name"><h5><b>Kumar</b></h5></div>
                    <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
            </div>

            <div class="card">
                    <div class="card-img"><img class="img" src="asset/img/male.png"></div>
                    <div class="card-name"><h5><b>Kumar</b></h5></div>
                    <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
            </div>

            <div class="card">
                    <div class="card-img"><img class="img" src="asset/img/male.png"></div>
                    <div class="card-name"><h5><b>Kumar</b></h5></div>
                    <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
            </div>

            <div class="card">
                    <div class="card-img"><img class="img" src="asset/img/male.png"></div>
                    <div class="card-name"><h5><b>Kumar</b></h5></div>
                    <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
            </div>
    </div>
</div>

<div class="row">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
                <div class="card-heading long-heading"><h4>Joint Secretaries</h4></div>
                <div class="card">
                        <div class="card-img"><img class="img" src="asset/img/male.png"></div>
                        <div class="card-name"><h5><b>Kumar</b></h5></div>
                        <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
                </div>
                <div class="card">
                        <div class="card-img"><img class="img" src="asset/img/male.png"></div>
                        <div class="card-name"><h5><b>Kumar</b></h5></div>
                        <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
                </div>
        </div>
</div>

<div class="row">
        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 text-center">
            <div class="card">
                <div class="card-heading"><h4>Editor</h4></div>
                <div class="card-img"><img class="img" src="asset/img/male.png"></div>
                <div class="card-name"><h5><b>Kumar</b></h5></div>
                <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
            </div>
        </div>
        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 text-center">
                <div class="card">
                        <div class="card-heading"><h4>Treasurer</h4></div>
                        <div class="card-img"><img class="img" src="asset/img/male.png"></div>
                        <div class="card-name"><h5><b>Kumar</b></h5></div>
                        <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
                </div>
        </div>
</div>

<div class="row">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
                <div class="card-heading long-heading"><h4>Consultative Committee</h4></div>
                <div class="card">
                        <div class="card-img"><img class="img" src="asset/img/male.png"></div>
                        <div class="card-name"><h5><b>Kumar</b></h5></div>
                        <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
                </div>
    
                <div class="card">
                        <div class="card-img"><img class="img" src="asset/img/male.png"></div>
                        <div class="card-name"><h5><b>Kumar</b></h5></div>
                        <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
                </div>
    
                <div class="card">
                        <div class="card-img"><img class="img" src="asset/img/male.png"></div>
                        <div class="card-name"><h5><b>Kumar</b></h5></div>
                        <div class="card-description"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been</p></div>
                </div>
        </div>
    </div>

    <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 commettiee">
                    <div class="card-heading long-heading"><h4>EXECUTIVE COMMITTEE</h4></div>
                    <ul>
                        <li><b>Dr. S. Pari Parameswaran</b><br>
                            <p>Professor of Youth Welfare, Department of Youth Welfare Studies, M.K University, Madurai. 625021, Email. youthparimku@yahoo.in , Mob: 09842113616</p>
                        </li>
                        <li><b>Dr. S. Pari Parameswaran</b><br>
                            <p>Professor of Youth Welfare, Department of Youth Welfare Studies, M.K University, Madurai. 625021, Email. youthparimku@yahoo.in , Mob: 09842113616</p>
                        </li>
                        <li><b>Dr. S. Pari Parameswaran</b><br>
                            <p>Professor of Youth Welfare, Department of Youth Welfare Studies, M.K University, Madurai. 625021, Email. youthparimku@yahoo.in , Mob: 09842113616</p>
                        </li>
                        <li><b>Dr. S. Pari Parameswaran</b><br>
                            <p>Professor of Youth Welfare, Department of Youth Welfare Studies, M.K University, Madurai. 625021, Email. youthparimku@yahoo.in , Mob: 09842113616</p>
                        </li>
                        <li><b>Dr. S. Pari Parameswaran</b><br>
                            <p>Professor of Youth Welfare, Department of Youth Welfare Studies, M.K University, Madurai. 625021, Email. youthparimku@yahoo.in , Mob: 09842113616</p>
                        </li>
                        <li><b>Dr. S. Pari Parameswaran</b><br>
                            <p>Professor of Youth Welfare, Department of Youth Welfare Studies, M.K University, Madurai. 625021, Email. youthparimku@yahoo.in , Mob: 09842113616</p>
                        </li>
                        <li><b>Dr. S. Pari Parameswaran</b><br>
                            <p>Professor of Youth Welfare, Department of Youth Welfare Studies, M.K University, Madurai. 625021, Email. youthparimku@yahoo.in , Mob: 09842113616</p>
                        </li>
                        <li><b>Dr. S. Pari Parameswaran</b><br>
                            <p>Professor of Youth Welfare, Department of Youth Welfare Studies, M.K University, Madurai. 625021, Email. youthparimku@yahoo.in , Mob: 09842113616</p>
                        </li>
                        <li><b>Dr. S. Pari Parameswaran</b><br>
                            <p>Professor of Youth Welfare, Department of Youth Welfare Studies, M.K University, Madurai. 625021, Email. youthparimku@yahoo.in , Mob: 09842113616</p>
                        </li>
                        <li><b>Dr. S. Pari Parameswaran</b><br>
                            <p>Professor of Youth Welfare, Department of Youth Welfare Studies, M.K University, Madurai. 625021, Email. youthparimku@yahoo.in , Mob: 09842113616</p>
                        </li>
                        <li><b>Dr. S. Pari Parameswaran</b><br>
                            <p>Professor of Youth Welfare, Department of Youth Welfare Studies, M.K University, Madurai. 625021, Email. youthparimku@yahoo.in , Mob: 09842113616</p>
                        </li>
                        <li><b>Dr. S. Pari Parameswaran</b><br>
                            <p>Professor of Youth Welfare, Department of Youth Welfare Studies, M.K University, Madurai. 625021, Email. youthparimku@yahoo.in , Mob: 09842113616</p>
                        </li>
                        <li><b>Dr. S. Pari Parameswaran</b><br>
                            <p>Professor of Youth Welfare, Department of Youth Welfare Studies, M.K University, Madurai. 625021, Email. youthparimku@yahoo.in , Mob: 09842113616</p>
                        </li>
                        <li><b>Dr. S. Pari Parameswaran</b><br>
                            <p>Professor of Youth Welfare, Department of Youth Welfare Studies, M.K University, Madurai. 625021, Email. youthparimku@yahoo.in , Mob: 09842113616</p>
                        </li>
                    </ul>
            </div>
    </div>
</div>